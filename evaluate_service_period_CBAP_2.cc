...
//                                                                   bnodeWifiMac->GetAssociationID (), allocationStart, true);  //used
//      allocationStart = apWifiMac->AllocateBeamformingServicePeriod (anodeWifiMac->GetAssociationID (),
//                                                                     cnodeWifiMac->GetAssociationID (), allocationStart, true);
//      allocationStart = apWifiMac->AllocateBeamformingServicePeriod (anodeWifiMac->GetAssociationID (),
...
//                                                                     cnodeWifiMac->GetAssociationID (), allocationStart, true);  //used
...
//  wireless communication point
          ...
          /* SP3 */
          apWifiMac->AllocateSingleContiguousBlock (3, CBAP_ALLOCATION, true,
                                                    AID_AP,
                                                    bnodeWifiMac->GetAssociationID (),
...
main (int argc, char *argv[])
{
  ...
  positionAlloc->Add (Vector (-1.0, 0.0, 0.0));   /* anode STA */ //staInterfaces.GetAddress (0)
  positionAlloc->Add (Vector (0.0, -1.0, 0.0));   /* bnode STA */ //staInterfaces.GetAddress (1)
  positionAlloc->Add (Vector (1.0, 0.0, 0.0));   /* cnode STA */ //staInterfaces.GetAddress (2)
  ...
  ApplicationContainer sinks = sinkHelper.Install (NodeContainer (cnodeNode, anodeNode, bnodeNode));
...
///  wirelesss point 
  ...
  OnOffHelper src;
  src.SetAttribute ("Remote", AddressValue (InetSocketAddress (staInterfaces.GetAddress (0), 9999)));  // 1=bnode 2=cnode
  ...
  srcApp.Add (src.Install (bnodeNode));
