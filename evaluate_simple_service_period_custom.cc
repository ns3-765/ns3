 ...
 * ./waf --run "evaluate_simple_service_period_custom"
 *
 * To run the script with different duration for the service period e.g. SP1=10ms:
 * ./waf --run "evaluate_simple_service_period_custom --spDuration=10000"
 *
 * Simulation Output:
...
Ptr<CodebookAnalytical> codebook;
  ...
  Ptr<WifiNetDevice> staWifiNetDevice = StaticCast<WifiNetDevice> (staDevices.Get (0));
